...
# ── Raid Timeout Configuration ──────────────────────────────────────────────
RAID_TIMEOUT_MINUTES = 120  # Auto-end raid after 120 minutes of inactivity

# ── Global Raid Lock ──────────────────────────────────────────────────────────
active_hosts = set()
setup_messages = {}  # Track setup messages per guild


def is_user_hosting(user_id: int) -> bool:
    """Check if a user is currently hosting a raid anywhere"""
    return user_id in active_hosts


def add_hosting_user(user_id: int):
    """Add a user to the active hosts set"""
    active_hosts.add(user_id)


def remove_hosting_user(user_id: int):
    """Remove a user from the active hosts set"""
    active_hosts.discard(user_id)


# ── XP Configuration ────────────────────────────────────────────────────────
def get_xp_for_wave_sync(wave: int) -> int:
    """Calculate XP for a single wave using exponential scaling."""
    return int(25 * (1.12 ** (wave - 1)))


def get_total_xp_for_raid_sync(final_wave: int) -> int:
    """Calculate total XP awarded for completing a raid up to the final wave."""
    total = 0
...
async def send_raid_log(bot: commands.Bot, embed: discord.Embed, view=None):
    """Send a raid log to the raid log channel"""
...
def get_raid(guild_id: int) -> dict:
    """Get the active raid for a guild from memory"""
    return raids.get(guild_id)


def raid_exists(guild_id: int) -> bool:
    """Check if a raid is active in a guild in memory"""
...
async def get_active_raid_from_db(guild_id: int) -> dict:
    """Get an active raid from the database (not completed)"""
    try:
...
async def is_raid_active_in_db(guild_id: int) -> bool:
    """Check if there's an active raid in the database"""
    try:
        result = await d1_query(
...
async def build_gate_embed(guild: discord.Guild, raid: dict) -> discord.Embed:
    """Build the public raid gate embed - NO Roblox usernames shown"""
    hosters = []
...
async def save_raid_to_history(guild_id: int, raid_data: dict, final_wave: int,
    ...
    """Save completed raid to history database"""
    try:
...
# ── Raid Timeout Functions ──────────────────────────────────────────────────
async def auto_end_raid(guild_id: int, bot: commands.Bot):
    """Auto-end a raid due to inactivity"""
    ...
    # Process rewards (no flagging for auto-end)
...
async def end_raid_logic(guild: discord.Guild, raid: dict, final_wave: int, flagged: bool, bot: commands.Bot):
    """Core raid ending logic without interaction dependency"""
    ...
    if not flagged:
        for hid in raid["hosters"]:
            ...
            # Award XP using levels.py (this sends level-up messages)
    ...
    if gate_ch:
        ...
        if not flagged:
            ...
            finished_embed.add_field(
                ...
                value=f"Avg **{mins_per_wave:.2f}** min/wave — under 1.5 min threshold.\nCheck the raid log channel.",
                inline=False
            )
    ...
    # Update top hoster roles
    await update_top_hoster_roles(guild, bot)
    
    # Remove hosts from active hosts
    for hid in raid["hosters"]:
        remove_hosting_user(hid)
    
    # Remove raid from memory
...
async def delayed_raid_timeout(guild_id: int, bot: commands.Bot):
    """Wait for timeout duration then end the raid"""
...
def update_raid_activity(guild_id: int):
    """Update the last activity time for a raid"""
    raid = get_raid(guild_id)
    if raid:
...
class FlaggedApprovalView(discord.ui.View):
    ...
    async def deny(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        log_embed.add_field(name="Wave", value=f"{self.final_wave}/20", inline=True)
        log_embed.add_field(name="Duration", value=self.duration, inline=True)
        log_embed.add_field(name="Reason", value="Suspicious wave speed (under 1.5 min/wave)", inline=True)
        await send_raid_log(interaction.client, log_embed)
...
class StartRaidModal(discord.ui.Modal, title="Start Raid"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # ── Public announcement in gate channel ─────────────────────────────
...
class JoinRaidModal(discord.ui.Modal, title="Join Raid"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # ── Send PS code as a private ephemeral embed in the gate channel ──
        ...
        # This sends a private embed that only the user can see in the channel
...
async def end_raid(interaction: discord.Interaction, final_wave: int):
    """End a raid and process rewards"""
    guild_id = interaction.guild_id
    raid = get_raid(guild_id)
...
class RaidSetupView(discord.ui.View):
    ...
    async def update_setup_embed(self, interaction: discord.Interaction):
        """Update the setup embed with current hosters"""
        embed = discord.Embed(
            title="⚔️ Raid Setup",
            color=discord.Color.blurple(),
            timestamp=datetime.now(timezone.utc)
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # ── Update the existing setup message to Confirmed ────────────────────
        embed = discord.Embed(
            title="⚔️ Raid Setup",
            color=discord.Color.green(),
            timestamp=datetime.now(timezone.utc)
        ...
        # Update the existing message (don't delete it)
        await interaction.message.edit(embed=embed, view=self)

        # ── Clean up OLD setup messages (from previous raids) ──────────────────
        host_ch = interaction.guild.get_channel(HOST_CHANNEL_ID)
        if host_ch:
            # Don't delete the current message - only delete tracked ones that aren't this one
            if guild_id in setup_messages:
                for msg_id in setup_messages[guild_id]:
                    if msg_id != interaction.message.id:
                        try:
                            ...
                            pass
                setup_messages[guild_id] = [interaction.message.id]  # Keep current message

        # ── Send Public Gate Message ────────────────────────────────────────
        gate_ch = interaction.guild.get_channel(RAID_GATE_ID)
        if gate_ch:
            await gate_ch.purge(limit=100)
            ping_role = interaction.guild.get_role(PING_ROLE_ID)
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # ── Public cancellation message in gate ─────────────────────────────
        gate_ch = interaction.guild.get_channel(RAID_GATE_ID)
        if gate_ch:
...
class Raid(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        # Register persistent views only (timeout=None with custom_id set)
    ...
    async def members(self, interaction: discord.Interaction):
        """List all members in the active raid with their Roblox usernames"""
        if not is_staff(interaction.user):
            return await interaction.response.send_message(
        ...
        # ── Get all members with their Roblox usernames ──────────────────────
        all_members = []
        
        # Get hosters with their Roblox usernames
        for hid in raid.get("hosters", []):
            member = interaction.guild.get_member(hid)
            if member:
                # Get Roblox username from database
                user_result = await d1_query(
                    "SELECT roblox_users FROM users WHERE discord_id = ?",
                    [str(hid)]
                )
        ...
        # Get players with their Roblox usernames
        for pid in raid.get("player_ids", []):
            # Skip if already added as hoster
        ...
        # ── Build the embed ──────────────────────────────────────────────────
        # Sort: Hosters first (with host at top), then players
